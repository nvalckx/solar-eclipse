...
export function drawSkyScene(
  ...
  const palette: Record<SkyState["twilightLevel"], [string, string, string]> = {
  ...
  if (
    ...
    if (state.eclipse.type === "total") {
      ...
      context.beginPath();
      context.arc(moonPoint.x, moonPoint.y, moonRadius * 1.02, 0, Math.PI * 2);
      context.stroke();
    }
  }
  ...
  if (mode === "sky") {
    ...
    if (showGolfHole) {
      ...
      const groundY =
        horizon + 20 + Math.sin(flagX / 47) * 8 + Math.sin(flagX / 17) * 2.5;
